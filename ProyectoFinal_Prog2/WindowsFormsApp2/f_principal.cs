...
namespace WindowsFormsApp2
{
    ...
    public partial class f_principal : f_base
    {
        ...
        struct DataEditoriales
        {
            public long IdEditorial; // { get; set; }
            public string nombreEditorial { get; set; }
            public string duennoEditorial { get; set; }
            public int AnnoEditorial { get; set; }
        }
        ...
        private void BTN_Calcular_Click(object sender, EventArgs e)
        {
            try
            {
                foreach(var Checkeado in checkedListBox1.CheckedItems)
                {
                    DataRowView row = (DataRowView)Checkeado;
                    int elID = Convert.ToInt32(row["id"]); //EL ID de la Base de datos
                }
                //for (int i = 0; i < checkedListBox1.CheckedItems.Count; i++) 
                //{
                //    DataRowView row = (DataRowView)checkedListBox1.CheckedItems[i];
                //    int elID = Convert.ToInt32(row["id"]);
                //}
                IncluirNuevoUsuario();
        ...
        {
            List<DataEditoriales> datos = new List<DataEditoriales>();
            foreach(cs_Editorial Editorial in cs_Editorial.getAllFromBD())
            {
                //CB_Editorial.Items.Add(Editorial.Name);
                datos.Add
                    (new DataEditoriales
                    {
                        IdEditorial = Editorial.Id,
                        nombreEditorial = Editorial.Name,
                        duennoEditorial = Editorial.Duenno,
                        AnnoEditorial = Editorial.Anno
                    });

            }
            CB_Editorial.DataSource = datos;
            CB_Editorial.DisplayMember = "nombreEditorial";
            //----------------

            //para un ChecklistBox
            ((ListBox)checkedListBox1).DataSource = cs_Editorial.traerTablita(); //DataTable
        ...
        {
            f_datagridSQL FDT = new f_datagridSQL();
            FDT.ShowDialog();
        }
    }
}
